...
class OverlayRenderer:
    ...
    transient Qt GraphicsItems (handles, ghosts, guides) onto a QGraphicsScene.
    """
    ...
    def _on_selection_changed(self, selected_node_ids: list[str]):
        """Reactive handler for drawing resize handles."""
        self.clear_handles()
    ...
    def _fig_to_scene(self, fig_pos: tuple[float, float]) -> QPointF:
        ...
        MANUAL LINEAR INTERPOLATION: Guaranteed parity with widget dimensions.
        """
        # Get actual logical pixels from the canvas widget
        width, height = self._figure.canvas.get_width_height()
        
        # fig_pos: (x_fraction, y_fraction)
        # x: 0 -> 0, 1 -> width
        # y: 0 (bottom) -> height (qt_y), 1 (top) -> 0 (qt_y)
