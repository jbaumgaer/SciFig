...
def mock_app_model():
    ...
    model.scene_root = MagicMock(spec=GroupNode) # Mock scene_root as it might be accessed
    model.layoutConfigChanged = MagicMock() # Mock signal
    return model
...
def mock_config_service():
    ...
    # Configure get method for QSettings initialization and _add_to_recent_files
...
@pytest.fixture
def project_controller(mock_app_model, mock_command_manager, mock_config_service, mock_layout_manager):
    return ProjectController(mock_app_model, mock_command_manager, mock_config_service, mock_layout_manager)

def test_project_controller_initialization(project_controller, mock_app_model, mock_command_manager, mock_config_service, mock_layout_manager):
    """
    Test that ProjectController initializes correctly and its attributes are set.
    """
    assert project_controller.model is mock_app_model
...
def test_save_project_calls_qfiledialog(project_controller, mock_parent_widget):
    """
...
def test_save_project_success(project_controller, mock_app_model, mock_config_service, mock_parent_widget, mock_plot_node):
    """
    ...
    mock_app_model.scene_root.all_descendants.return_value = [mock_plot_node]
    mock_app_model.to_dict.return_value = {"version": "1.0", "scene": {}}
    ...
    with patch('PySide6.QtWidgets.QFileDialog.getSaveFileName', return_value=(mock_file_path, "")), \
        ...
        mock_app_model.to_dict.assert_called_once()
        mock_json_dump.assert_called_once_with(mock_app_model.to_dict.return_value, mock_builtin_open(), indent=4)
...
def test_save_project_cancellation(project_controller, mock_app_model, mock_parent_widget):
    """
    ...
    with patch('PySide6.QtWidgets.QFileDialog.getSaveFileName', return_value=("", "")) as mock_get_save_file_name, \
        ...
        mock_app_model.to_dict.assert_not_called()
        MockPath_constructor.assert_not_called()


def test_open_project_calls_qfiledialog(project_controller, mock_parent_widget):
    """
...
def test_open_project_success(project_controller, mock_app_model, mock_parent_widget, mock_plot_node):
    """
    ...
    with patch('PySide6.QtWidgets.QFileDialog.getOpenFileName', return_value=(mock_file_path, "")) as mock_get_open_file_name, \
        ...
        mock_app_model.load_from_dict.assert_called_once_with(mock_project_dict, mock_temp_dir_path_instance)
...
def test_create_new_layout_sets_scene_root(project_controller, mock_app_model, mock_config_service):
    """
    ...
    mock_new_plot_node = MagicMock(spec=PlotNode, id="plot_001") # For all_descendants
    ...
    with patch('src.controllers.project_controller.Path') as MockPath_constructor, \
        ...
        mock_app_model.set_scene_root.assert_called_once_with(mock_new_root_node)

        # Ensure existing plot data is processed (even if empty in this test)
        mock_app_model.scene_root.all_descendants.assert_called() # Called to extract existing states
...
def test_open_project_error_logs_exception(project_controller, mock_app_model, mock_parent_widget, caplog):
    """
    ...
    with patch('PySide6.QtWidgets.QFileDialog.getOpenFileName', return_value=(mock_file_path, "")) as mock_get_open_file_name, \
        ...
        mock_app_model.load_from_dict.assert_not_called()
        mock_add_to_recent_files.assert_not_called()

def test_open_project_cancellation(project_controller, mock_app_model, mock_parent_widget):
    """
