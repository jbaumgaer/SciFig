...
class TestProjectController:
    ...
    def test_project_controller_initialization(
        self,
        project_controller,
        mock_application_model,
        mock_command_manager,
    ...
    def test_save_project_calls_qfiledialog(
        self, project_controller, mock_parent_widget
    ):
        """
    ...
    def test_save_project_success(
        ...
        with (
            ...
            patch.object(
                project_controller, "_add_to_recent_files"
            ) as mock_add_to_recent_files,
            patch("src.controllers.project_controller.Path") as MockPath_constructor,
            patch("os.mkdir") as mock_os_mkdir,
        ):  # <--- Patch os.mkdir to prevent actual directory creation

            # The string path returned by TemporaryDirectory
            temp_dir_str_from_context = "/mock/temp/dir_abc"
            mock_temp_dir_class.return_value.__enter__.return_value = (
                temp_dir_str_from_context
            )
            ...
            mock_temp_dir_path_instance.exists.return_value = (
                True  # Important for mkdir behavior if parents=False
            )

            # Mock the .rglob() result
            mock_project_json_path = MagicMock(spec=Path)
            mock_project_json_path.__str__.return_value = (
                f"{temp_dir_str_from_context}/project.json"
            )
            # Mock relative_to for the project_json_path mock
            mock_project_json_path.relative_to.return_value = MagicMock(
                spec=Path, __str__=lambda: "project.json"
            )

            mock_parquet_file_path = MagicMock(spec=Path)
            mock_parquet_file_path.__str__.return_value = (
                f"{temp_dir_str_from_context}/data/{mock_plot_node.id}.parquet"
            )
            # Mock relative_to for the parquet_file_path mock
            mock_parquet_file_path.relative_to.return_value = MagicMock(
                spec=Path, __str__=lambda: f"data/{mock_plot_node.id}.parquet"
            )

            mock_temp_dir_path_instance.rglob.return_value = [
                mock_project_json_path,
                mock_parquet_file_path,
            ]

            # Mocking for data_dir = temp_dir / "data"
            mock_data_dir_path_instance = MagicMock(spec=Path)
            mock_data_dir_path_instance.mkdir.return_value = (
                None  # Ensure mkdir on data_dir succeeds
            )
            mock_data_dir_path_instance.__str__.return_value = (
                f"{temp_dir_str_from_context}/data"
            )

            # Mocking for parquet_path = data_dir / f"{node.id}.parquet"
            mock_final_parquet_path_instance = MagicMock(spec=Path)
            mock_final_parquet_path_instance.__str__.return_value = (
                f"{temp_dir_str_from_context}/data/{mock_plot_node.id}.parquet"
            )
            ...
            mock_data_dir_path_instance.__truediv__.side_effect = lambda other: (
                mock_final_parquet_path_instance
                if str(other) == f"{mock_plot_node.id}.parquet"
                else MagicMock(spec=Path)
            )
            ...
            # Assertions
            MockPath_constructor.assert_any_call(
                temp_dir_str_from_context
            )  # Verify Path() constructor was called for temp_dir

            mock_data_dir_path_instance.mkdir.assert_called_once()

            mock_plot_node.data.to_parquet.assert_called_once_with(
                mock_final_parquet_path_instance
            )  # Use the specific mock path

            mock_application_model.to_dict.assert_called_once()
            mock_json_dump.assert_called_once_with(
                mock_application_model.to_dict.return_value,
                mock_builtin_open(),
                indent=4,
            )
            mock_zip_file_class.assert_called_once_with(
                mock_file_path, "w", zipfile.ZIP_DEFLATED
            )

            zip_file_mock_instance = (
                mock_zip_file_class.return_value.__enter__.return_value
            )
    ...
    def test_save_project_cancellation(
        self, project_controller, mock_application_model, mock_parent_widget
    ):
        """
    ...
    def test_open_project_calls_qfiledialog(
        self, project_controller, mock_parent_widget
    ):
        """
    ...
    def test_open_project_success(
        ...
        with (
            ...
            patch("tempfile.TemporaryDirectory") as mock_temp_dir_class,
            patch(
                "builtins.open", mock_open(read_data=json.dumps(mock_project_dict))
            ...
            mock_temp_dir_path_instance.__str__.return_value = temp_dir_str_from_context
            mock_temp_dir_path_instance.exists.return_value = (
                True  # Important for Path.exists() if called
            )

            # Mock project.json path
            mock_json_path_instance = MagicMock(spec=Path)
            mock_json_path_instance.__str__.return_value = (
                f"{temp_dir_str_from_context}/project.json"
            )
            ...
            mock_zip_file_class.return_value.__enter__.return_value.extractall.return_value = (
                None
            )
            mock_zf_bound_instance = (
                mock_zip_file_class.return_value.__enter__.return_value
            )  # Reference the object 'zf' binds to
            ...
            mock_zf_bound_instance.extractall.assert_called_once_with(
                mock_temp_dir_path_instance
            )  # Assert on the correctly bound mock instance

            # Assert project.json was loaded
            mock_json_load.assert_called_once_with(mock_builtin_open())

            # Assert model was loaded from dict
            mock_application_model.load_from_dict.assert_called_once_with(
                mock_project_dict, mock_temp_dir_path_instance
            )
    ...
    def test_create_new_layout_sets_scene_root(
        self, project_controller, mock_application_model, mock_config_service
    ):
        """
        ...
        mock_layout_template_data = {
            "id": "new_root_layout",
            "type": "group",
            "name": "Test Layout",
            "visible": True,
        }  # <--- ADDED "visible"
        mock_new_root_node = MagicMock(
            spec=GroupNode, id="new_root_layout", name="New Root Layout"
        )
        mock_new_plot_node = MagicMock(
            spec=PlotNode, id="plot_001", name="New Plot Node"
        )  # For all_descendants
        ...
        with (
            patch("src.controllers.project_controller.Path") as MockPath_constructor,
            patch(
                "json.load", return_value=mock_layout_template_data
            ) as mock_json_load,
            patch(
                "src.controllers.project_controller.node_factory",
                return_value=mock_new_root_node,
            ) as mock_node_factory,
            patch("builtins.open", mock_open()) as mock_builtin_open,
        ):  # <--- ADD THIS LINE
            ...
            MockPath_constructor.return_value = (
                mock_template_path_instance  # default return for Path()
            )

            # Ensure that Path(layout_templates_dir) / default_template_name works
            mock_layout_templates_dir_path = MagicMock(spec=Path)
            mock_layout_templates_dir_path.__truediv__.return_value = (
                mock_template_path_instance
            )
            ...
            mock_new_root_node.all_descendants.return_value = [
                mock_new_plot_node
            ]  # Simulate plots in new layout

            project_controller.create_new_layout()

            # Assertions
            mock_config_service.get.assert_any_call(
            ...
            mock_application_model.set_scene_root.assert_called_once_with(
                mock_new_root_node
            )
    ...
    def test_open_project_error_logs_exception(
        self, project_controller, mock_application_model, mock_parent_widget, caplog
    ):
        """
        ...
        with (
            ...
            mock_zip_file_class.side_effect = zipfile.BadZipFile(
                "Simulated open zip error"
            )
            ...
            mock_temp_dir_path_instance.__str__.return_value = temp_dir_str_from_context
            MockPath_constructor.return_value = (
                mock_temp_dir_path_instance  # ensure Path() call returns our mock
            )
            ...
            with caplog.at_level(logging.ERROR):
                project_controller.open_project(parent=mock_parent_widget)
                assert (
                    "Error opening project file 'error_open_test.sci': Simulated open zip error"
                    in caplog.text
                )
            ...
            mock_zip_file_class.assert_called_once_with(
                mock_file_path, "r"
            )  # ZipFile is called before error
    ...
    def test_open_project_cancellation(
        self, project_controller, mock_application_model, mock_parent_widget
    ):
        """
