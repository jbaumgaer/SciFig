...
class TestThemeManager:
    ...
    def test_init_loads_themes(self, mock_qapplication, mock_config_service, sample_themes_yaml):
        ...
        with patch("builtins.open", mock_open(read_data=yaml_str)):
            with patch("pathlib.Path.exists", return_value=True):
                manager = ThemeManager(mock_qapplication, mock_config_service)
                
        assert "dark" in manager.get_available_themes()
        assert "light" in manager.get_available_themes()

    def test_apply_theme_success(self, mock_qapplication, mock_config_service, sample_themes_yaml):
        ...
        mock_qapplication.setStyle.assert_called_with("Fusion")
        mock_qapplication.setPalette.assert_called()
        mock_qapplication.setStyleSheet.assert_called_with("QPushButton { color: white; }")

    def test_apply_theme_fallback_to_default(self, mock_qapplication, mock_config_service, sample_themes_yaml, caplog):
        ...
        mock_qapplication.setStyle.assert_called_with("Windows")

    def test_load_themes_file_not_found(self, mock_qapplication, mock_config_service):
    ...
    def test_palette_application_details(self, mock_palette_cls, mock_qcolor_cls, mock_qapplication, mock_config_service, sample_themes_yaml):
