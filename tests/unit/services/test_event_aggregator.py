...
class TestEventAggregator:
    ...
    def test_publish_triggers_subscribed_handler(self, real_event_aggregator):
        """Verifies that a basic subscription results in a call upon publication."""
        handler = MagicMock()
        handler.__name__ = "mock_handler"
        real_event_aggregator.subscribe(Events.SELECTION_CHANGED, handler)
        
        real_event_aggregator.publish(Events.SELECTION_CHANGED, ["id1", "id2"])
        
        handler.assert_called_once_with(["id1", "id2"])

    def test_publish_notifies_multiple_subscribers(self, real_event_aggregator):
        ...
        real_event_aggregator.subscribe(Events.PROJECT_OPENED, handler1)
        real_event_aggregator.subscribe(Events.PROJECT_OPENED, handler2)
        
        real_event_aggregator.publish(Events.PROJECT_OPENED, "test.sci")
    ...
    def test_publish_passes_positional_and_keyword_arguments(self, real_event_aggregator):
    ...
    def test_publish_to_no_subscribers_is_safe(self, real_event_aggregator):
        """Ensures that publishing an event with no listeners doesn't raise errors."""
        # This should execute without exception
        real_event_aggregator.publish(Events.SAVE_PROJECT_REQUESTED)

    def test_handler_exception_isolation(self, real_event_aggregator, caplog):
        """
        ...
        real_event_aggregator.subscribe(Events.SELECTION_CHANGED, handler_fail)
        real_event_aggregator.subscribe(Events.SELECTION_CHANGED, handler_ok)
        
        # Publish with logging captured
        with caplog.at_level(logging.ERROR):
            real_event_aggregator.publish(Events.SELECTION_CHANGED, ["p1"])
    ...
    def test_invalid_event_type_logs_error(self, real_event_aggregator, caplog):
        ...
        with caplog.at_level(logging.ERROR):
            ...
            real_event_aggregator.subscribe("NOT_AN_ENUM", handler)
            assert "Attempted to subscribe to invalid event type" in caplog.text
            
            # 2. Test invalid publish
            real_event_aggregator.publish("NOT_AN_ENUM")
            assert "Attempted to publish invalid event type" in caplog.text

    # --- State Tracking ---

    def test_publish_count_increment(self, real_event_aggregator):
        ...
        real_event_aggregator.publish(Events.SELECTION_CHANGED, [])
        assert real_event_aggregator._publish_count == 1
        
        real_event_aggregator.publish(Events.SELECTION_CHANGED, [])
        assert real_event_aggregator._publish_count == 2
