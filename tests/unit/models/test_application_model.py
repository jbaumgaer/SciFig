...
class TestApplicationModel:
    ...
    def test_initialization(
        self, application_model_with_mocks, mock_figure, mock_config_service
    ):
    ...
    def test_add_node_to_specific_parent(
        self, application_model_with_mocks, mock_scene_node, mock_group_node
    ):
    ...
    def test_set_scene_root(
        self, application_model_with_mocks, mock_group_node, mock_scene_node
    ):
    ...
    def test_current_layout_config_setter_getter(
        self, application_model_with_mocks, new_config_instance
    ):
        ...
        initial_config_is_free_instance = isinstance(
            model.current_layout_config, FreeConfig
        )
        new_config_is_free_instance = isinstance(new_config_instance, FreeConfig)

        if (
            initial_config_is_free_instance
            and new_config_is_free_instance
            and model.current_layout_config == new_config_instance
        ):
    ...
    def test_get_node_at(
        self, application_model_with_mocks, mock_group_node, mock_scene_node, mocker
    ):
        ...
        mock_hit_test = mocker.patch.object(
            model.scene_root, "hit_test", return_value=None
        )
    ...
    def test_to_dict_with_nodes_and_layout(self, application_model_with_mocks, mocker):
        ...
        mocker.patch.object(
            ...
            return_value={
                ...
                "name": "root",
                "visible": True,
                "children": [real_scene_node.to_dict(), real_group_node.to_dict()],
            },
        )
        ...
        assert (
            model_dict["layout_config"] == grid_config_instance.to_dict()
        )  # Use actual to_dict output
    ...
    def test_load_from_dict_empty_data(self, application_model_with_mocks, mocker):
        ...
        node_factory_patch = mocker.patch(
            "src.models.application_model.node_factory", return_value=mock_new_root_node
        )

        # We expect LayoutConfig.from_dict to NOT be called, as FreeConfig() is directly assigned
        layout_config_from_dict_patch = mocker.patch(
            "src.models.layout.layout_config.LayoutConfig.from_dict"
        )
        ...
        assert isinstance(
            model.current_layout_config, FreeConfig
        )  # Assert type, not identity

        # Assert signal call counts
        model.selectionChanged.emit.assert_called_once()  # clear_scene -> set_selection
        assert (
            model.modelChanged.emit.call_count == 2
        )  # 1 from clear_scene + 1 from end of load_from_dict

        # Use the stored patch object for assertion
        node_factory_patch.assert_called_once_with(
            empty_data["scene_root"], temp_dir=Path(".")
        )
    ...
    def test_load_from_dict_with_data(self, application_model_with_mocks, mocker):
        ...
        node_factory_patch = mocker.patch(
            "src.models.application_model.node_factory", return_value=mock_loaded_root
        )
        ...
        layout_config_from_dict_patch = mocker.patch(
            "src.models.layout.layout_config.LayoutConfig.from_dict",
            return_value=mock_loaded_layout_config,
        )
        ...
        data = {
            ...
            "layout_config": {
                ...
                "rows": 2,
                "cols": 2,
            },  # This will trigger LayoutConfig.from_dict
        }
        temp_dir = Path("./temp")
        ...
        assert (
            model.modelChanged.emit.call_count == 3
        )  # 1 (clear_scene) + 1 (layout config setter) + 1 (end of load_from_dict)

        # Verify node_factory was called correctly
        node_factory_patch.assert_called_once_with(
            data["scene_root"], temp_dir=temp_dir
        )
    ...
    def test_load_from_dict_missing_layout_config(
        self, application_model_with_mocks, mocker
    ):
        ...
        assert isinstance(
            model.current_layout_config, FreeConfig
        )  # It should be a *real* FreeConfig instance now
        ...
        assert (
            model.modelChanged.emit.call_count == 2
        )  # 1 (clear_scene) + 1 (end of load_from_dict)
    ...
    def test_serialization_round_trip(self, application_model_with_mocks, mocker):
        ...
        real_model = ApplicationModel(
            figure=MagicMock(spec=matplotlib.figure.Figure),
        ...
        node_factory_patch = mocker.patch(
            "src.models.application_model.node_factory",
            side_effect=scene_node.node_factory,
        )
        # Patch LayoutConfig.from_dict to return the *real* LayoutConfig.from_dict
        layout_config_from_dict_patch = mocker.patch(
            "src.models.layout.layout_config.LayoutConfig.from_dict",
            side_effect=LayoutConfig.from_dict,
        )
        ...
        assert (
            loaded_model.current_layout_config == grid_config
