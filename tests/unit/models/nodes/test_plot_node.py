...
from src.models.plots.plot_properties import (
    BasePlotProperties, LinePlotProperties, ScatterPlotProperties,
    PlotMapping, AxesLimits, PlotType
)
...
def mock_axes():
    ...
    mock_bbox = MagicMock()
    mock_bbox.x0, mock_bbox.y0, mock_bbox.x1, mock_bbox.y1 = 0.1, 0.1, 0.9, 0.9 # Default bbox covering most of figure
    ...
    mock_bbox.transformed.return_value = mock_bbox # Ensure transformed bbox retains attributes
...
class TestPlotNode:
    ...
    @pytest.mark.parametrize("position, expected_hit", [
    ...
    def test_hit_test_with_axes(self, simple_plot_node, mock_axes, position, expected_hit):
        """
    ...
    def test_to_dict_no_properties_no_data(self, simple_plot_node):
        ...
        assert node_dict["geometry"] == {"x": 0.1, "y": 0.1, "width": 0.8, "height": 0.8}
        assert node_dict["plot_properties"] is None
        assert node_dict["data_path"] is None
        assert node_dict["children"] == []

    def test_to_dict_with_properties_and_data(self, plot_node_with_properties, plot_node_with_data):
        ...
        assert node_dict["geometry"] == {"x": 0.1, "y": 0.1, "width": 0.8, "height": 0.8}
        assert node_dict["plot_properties"] == dataclasses.asdict(node_to_serialize.plot_properties)
    ...
    @pytest.mark.parametrize("plot_type_str, expected_prop_class", [
        ("line", LinePlotProperties),
        ("scatter", ScatterPlotProperties)
    ])
    def test_from_dict_with_plot_properties(self, tmp_path, plot_type_str, expected_prop_class):
        """
        ...
        plot_dict = {
            ...
            "plot_properties": {
                ...
                "marker_size": 25 if plot_type_str == "scatter" else None # Specific to scatter
            },
            "children": []
        }
