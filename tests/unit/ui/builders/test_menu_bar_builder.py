...
@pytest.fixture
def menu_bar_builder(
    mock_project_controller, mock_layout_controller, mock_command_manager
):
...
class TestMenuBarBuilder:
    def test_main_menu_actions_dataclass_structure(self):
        ...
        mock_objects = {
            field_name: Mock(spec=QAction if "action" in field_name else QMenu)
        ...
        for field_name in MainMenuActions.__annotations__.keys():
            assert hasattr(
                menu_actions, field_name
            ), f"MainMenuActions is missing attribute: {field_name}"
    ...
    def test_build_creates_all_file_menu_actions(self, menu_bar_builder):
        ...
        for attr_name in file_menu_action_attributes:
            ...
            assert isinstance(
                action, QAction
            ), f"Attribute {attr_name} is not a QAction"
    ...
    def test_menu_bar_builder_constructor_stores_dependencies(
        self, mock_project_controller, mock_layout_controller, mock_command_manager
    ):
    ...
    def test_build_file_menu_actions_and_shortcuts(self, menu_bar_builder):
        ...
        for attr_name, expected_text, expected_shortcut in expected_actions_data:
            ...
            assert (
                action.text() == expected_text
            ), f"Action {attr_name} has incorrect text"
            if expected_shortcut:
                assert (
                    action.shortcut() == expected_shortcut
                ), f"Action {attr_name} has incorrect shortcut"
    ...
    def test_build_file_menu_structure(self, menu_bar_builder):
        ...
        assert (
            menu_actions.export_vector_menu.parent() == menu_actions.export_figure_menu
        )
        ...
        assert (
            menu_actions.export_raster_menu.parent() == menu_actions.export_figure_menu
        )

    def test_file_menu_action_connections(
        self, menu_bar_builder, mock_project_controller, qtbot
    ):
    ...
    def test_edit_menu_action_connections(
        self, menu_bar_builder, mock_command_manager, qtbot
    ):
    ...
    def test_update_recent_projects_menu_with_files(
        self, menu_bar_builder, mock_project_controller, qtbot
    ):
    ...
    def test_update_recent_projects_menu_no_files(
        self, menu_bar_builder, mock_project_controller
    ):
    ...
    def test_build_returns_menu_bar_and_main_menu_actions(self, menu_bar_builder):
        ...
        for field_name in MainMenuActions.__annotations__.keys():
            ...
            ), f"MainMenuActions attribute {field_name} was not populated."
            if "menu" in field_name:
                assert isinstance(
                    attribute, QMenu
                ), f"Attribute {field_name} is not a QMenu"
            elif "action" in field_name:
                assert isinstance(
                    attribute, QAction
                ), f"Attribute {field_name} is not a QAction"
