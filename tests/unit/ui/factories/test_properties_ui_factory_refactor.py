...
def mock_properties_ui_factory_instance():
    ...
    return PlotPropertiesUIFactory()
...
class TestPropertiesUIFactoryRefactor:
    def test_factory_instantiation(self, mock_properties_ui_factory_instance):
        ...
        assert isinstance(factory, PlotPropertiesUIFactory)
    ...
    def test_build_widgets_with_unregistered_builder_no_error(
        self,
    ...
    def test_register_builder_overwrites_existing(
        self, mock_properties_ui_factory_instance, common_ui_args
    ):
    ...
    def test_build_widgets_full_integration(
        self,
        ...
        mock_build_base_plot_properties_ui.assert_called_once_with(
            node=mock_line_node, **common_ui_args
        )
        # These should now be called from within the side_effect of mock_build_base_plot_properties_ui
        mock_build_column_selectors.assert_called_once_with(
            node=mock_line_node,
            layout=common_ui_args["layout"],
            x_combo=common_ui_args["x_combo"],
            y_combo=common_ui_args["y_combo"],
        ...
        mock_build_limit_selectors.assert_called_once_with(
            node=mock_line_node,
            layout=common_ui_args["layout"],
            limit_edits=common_ui_args["limit_edits"],
            on_limit_editing_finished=common_ui_args["on_limit_editing_finished"],
        )
        mock_build_line_plot_ui_widgets.assert_called_once_with(
            node=mock_line_node, **common_ui_args
        )
        ...
        mock_build_base_plot_properties_ui.assert_called_once_with(
            node=mock_scatter_node, **common_ui_args
        )
        # These should now be called from within the side_effect of mock_build_base_plot_properties_ui
        mock_build_column_selectors.assert_called_once_with(
            node=mock_scatter_node,
        ...
        mock_build_scatter_plot_ui_widgets.assert_called_once_with(
            node=mock_scatter_node, **common_ui_args
        )
...
class TestStandaloneBuilders:
    ...
    def mock_limit_edits(self, mock_qlineedit):
        ...
        return {
            "xlim_min": Mock(
                spec=QLineEdit,
                editingFinished=MagicMock(),
                setValidator=MagicMock(),
                text=Mock(return_value="1.0"),
            ),
            "xlim_max": Mock(
                spec=QLineEdit,
                editingFinished=MagicMock(),
                setValidator=MagicMock(),
                text=Mock(return_value="5.0"),
            ),
            "ylim_min": Mock(
                spec=QLineEdit,
                editingFinished=MagicMock(),
                setValidator=MagicMock(),
                text=Mock(return_value="2.0"),
            ),
            "ylim_max": Mock(
                spec=QLineEdit,
                editingFinished=MagicMock(),
                setValidator=MagicMock(),
                text=Mock(return_value="6.0"),
            ),
        }

    def test_build_column_selectors(
        self,
        qtbot,
        mock_plot_node_with_data,
        mock_form_layout,
        mock_qcombobox,
    ...
    def test_build_limit_selectors(
        self,
        qtbot,
        mock_plot_node_with_data,
        mock_form_layout,
    ...
    @patch("src.views.properties_ui_factory._build_column_selectors")
    @patch("src.views.properties_ui_factory._build_limit_selectors")
    def test_build_base_plot_properties_ui(
        self,
        mock_build_limit_selectors,
        mock_build_column_selectors,
        qtbot,
        ...
        # Check for title
        assert any(
            args[0][0] == "Title:" and isinstance(args[0][1], QLineEdit)
            for args in mock_form_layout.addRow.call_args_list
        )

        # Check for xlabel
        assert any(
            args[0][0] == "X-Axis Label:" and isinstance(args[0][1], QLineEdit)
            for args in mock_form_layout.addRow.call_args_list
        )

        # Check for ylabel
        assert any(
            args[0][0] == "Y-Axis Label:" and isinstance(args[0][1], QLineEdit)
            for args in mock_form_layout.addRow.call_args_list
        )

    @patch("src.views.properties_ui_factory._build_base_plot_properties_ui")
    def test_build_line_plot_ui_widgets(
        self,
        mock_build_base_plot_properties_ui,
        qtbot,
        mock_line_node,
    ...
    @patch("src.views.properties_ui_factory._build_base_plot_properties_ui")
    def test_build_scatter_plot_ui_widgets(
        self,
        mock_build_base_plot_properties_ui,
        qtbot,
        mock_scatter_node,
        ...
        mock_build_base_plot_properties_ui.assert_called_once()
        assert any(
            "Marker Size" in str(args) and isinstance(args[0][1], QLineEdit)
            for args in mock_form_layout.addRow.call_args_list
        )
        # Assuming the QLineEdit text is set to the property value
        # This part requires a deeper inspection of the mock_form_layout or the widget itself
