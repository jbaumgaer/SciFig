...
from src.controllers.layout_controller import LayoutController # Changed from MainController
...
from src.models.layout.layout_config import GridConfig, Margins, Gutters # Import new config classes
...
class TestLayoutUIFactory:
    ...
    def test_build_layout_controls_free_form_mode(self, layout_ui_factory, mock_layout_controller, mock_layout_manager):
        """
        Test that build_layout_controls builds free form controls with icon buttons
        when ui_selected_layout_mode is FREE_FORM.
        """
        mock_layout_manager.ui_selected_layout_mode = LayoutMode.FREE_FORM # Set UI selected mode
        parent_widget = QWidget()
        controls = layout_ui_factory.build_layout_controls(mock_layout_controller, parent_widget)
    ...
    def test_build_layout_controls_grid_mode(self, layout_ui_factory, mock_layout_controller, mock_layout_manager):
        """
        Test that build_layout_controls builds grid layout controls when ui_selected_layout_mode is GRID.
        """
        mock_layout_manager.ui_selected_layout_mode = LayoutMode.GRID # Set UI selected mode
        mock_layout_manager._last_grid_config = GridConfig(rows=1, cols=1, margins=Margins(0,0,0,0), gutters=Gutters([],[])) # Provide a minimal config
        parent_widget = QWidget()
        controls = layout_ui_factory.build_layout_controls(mock_layout_controller, parent_widget)
        assert isinstance(controls, QWidget)
    ...
    def test_build_grid_layout_controls_no_active_config(self, layout_ui_factory, mock_layout_controller, mock_layout_manager):
        """
        ...
        parent_widget = QWidget()
        controls = layout_ui_factory._build_grid_layout_controls(mock_layout_controller, parent_widget)
        assert isinstance(controls, QWidget)
    ...
    def test_build_grid_layout_controls_with_active_config(self, layout_ui_factory, mock_layout_controller, mock_layout_manager):
        """
        ...
        active_grid_config = GridConfig(
            rows=3, cols=4, row_ratios=[1.0]*3, col_ratios=[1.0]*4,
            margins=Margins(top=0.1, bottom=0.2, left=0.3, right=0.4),
            gutters=Gutters(hspace=[0.05], wspace=[0.15])
        )
        mock_layout_manager._last_grid_config = active_grid_config # Set an active config
        parent_widget = QWidget()
        controls = layout_ui_factory._build_grid_layout_controls(mock_layout_controller, parent_widget)
    ...
    def test_handle_line_edit_change_valid_input(self, layout_ui_factory, mock_layout_controller, mock_layout_manager, qtbot):
        """
        ...
        mock_layout_manager._last_grid_config = GridConfig(
            rows=2, cols=2, row_ratios=[1.0, 1.0], col_ratios=[1.0, 1.0],
            margins=Margins(top=0.1, bottom=0.1, left=0.1, right=0.1),
            gutters=Gutters(hspace=[0.05], wspace=[0.05])
        )
        
        parent_widget = QWidget()
        rows_edit = layout_ui_factory._create_parameter_line_edit("rows", 2, QIntValidator(1, 99), parent_widget, mock_layout_controller)
        
        # Simulate user input
        rows_edit.setText("3")
        
        # Manually call the connected slot
        layout_ui_factory._handle_line_edit_change(mock_layout_controller, "rows", rows_edit)
        
        mock_layout_controller.on_grid_layout_param_changed.assert_called_once_with("rows", 3)


    def test_handle_line_edit_change_invalid_input_restores_value(self, layout_ui_factory, mock_layout_controller, mock_layout_manager, qtbot):
        """
    ...
    def test_handle_line_edit_change_invalid_input_no_config_clears(self, layout_ui_factory, mock_layout_controller, mock_layout_manager, qtbot):
        """
        ...
        parent_widget = QWidget()
        rows_edit = layout_ui_factory._create_parameter_line_edit("rows", None, QIntValidator(1, 99), parent_widget, mock_layout_controller)
