...
from src.shared.types import Rect
...
def test_free_layout_engine_aligns_geometries(real_free_layout_engine):
    ...
    plot1 = PlotNode()
    plot1.set_geometry(0.1, 0.4, 0.2, 0.2)
    plot2 = PlotNode()
    plot2.set_geometry(0.5, 0.1, 0.2, 0.2)
    plot3 = PlotNode()
    plot3.set_geometry(0.3, 0.7, 0.2, 0.2)
    ...
    max_right = max([p.x + p.width for p in plots])
    ...
    max_bottom = max([p.y + p.height for p in plots])
    ...
    min_x = min(p.x for p in plots)
    max_x_plus_width = max(p.x + p.width for p in plots)
    ...
    geometries_v_middle = engine.perform_align(plots, 'v_middle')
    min_y = min(p.y for p in plots)
    max_y_plus_height = max(p.y + p.height for p in plots)
    middle_y = (min_y + max_y_plus_height) / 2
    for plot_id, geom in geometries_v_middle.items():
        plot = next(p for p in plots if p.id == plot_id)
        assert (geom[1] + geom[3] / 2) == pytest.approx(middle_y, abs=1e-9)
...
def test_free_layout_engine_distributes_geometries(real_free_layout_engine):
    ...
    plot1 = PlotNode()
    plot1.set_geometry(0.1, 0.1, 0.1, 0.1) # x=0.1, width=0.1 -> right=0.2
    plot2 = PlotNode()
    plot2.set_geometry(0.5, 0.2, 0.2, 0.2) # x=0.5, width=0.2 -> right=0.7
    plot3 = PlotNode()
    plot3.set_geometry(0.8, 0.3, 0.1, 0.1) # x=0.8, width=0.1 -> right=0.9
    plots = [plot1, plot2, plot3]

    # Distribute Horizontally
    geometries_h = engine.perform_distribute(plots, 'horizontal')
    
    # Extract left-most (x) and right-most (x + width) coordinates
    plot_lefts = sorted([geometries_h[p.id][0] for p in plots])
    plot_rights = sorted([geometries_h[p.id][0] + geometries_h[p.id][2] for p in plots])
    
    # Calculate spacing between left edges
    spacing1 = plot_lefts[1] - plot_lefts[0]
    spacing2 = plot_lefts[2] - plot_lefts[1]
    
    # Expect approximately equal spacing (allow for float precision)
    assert abs(spacing1 - spacing2) < 1e-9

    # Distribute Vertically
    plot1.set_geometry(0.1, 0.1, 0.1, 0.1) # y=0.1, height=0.1 -> bottom=0.2
    plot2.set_geometry(0.2, 0.5, 0.2, 0.2) # y=0.5, height=0.2 -> bottom=0.7
    plot3 = PlotNode()
    plot3.set_geometry(0.3, 0.8, 0.1, 0.1) # y=0.8, height=0.1 -> bottom=0.9
    ...
    plot_bottoms = sorted([geometries_v[p.id][1] + geometries_v[p.id][3] for p in plots])
