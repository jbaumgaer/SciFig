...
@pytest.fixture
def real_layout_manager_for_commands(
    real_application_model_with_plot, mock_config_service
):
...
class TestCommandsIntegration:
    ...
    def test_change_property_command_execute_undo(
        self, real_application_model_with_plot, real_command_manager
    ):
        """
    ...
    def test_change_property_command_plot_type_transform(
        self, real_application_model_with_plot, real_command_manager
    ):
        """
    ...
    def test_batch_change_plot_geometry_command_execute_undo(
        self, real_application_model_with_multiple_plots, real_command_manager
    ):
        """
        ...
        initial_geometries = {plot1.id: plot1.geometry, plot2.id: plot2.geometry}
    ...
    def test_change_grid_parameters_command_execute_undo(
        self,
        real_application_model_with_plot,
        real_command_manager,
        real_layout_manager_for_commands,
    ):
        """
    ...
    def test_command_manager_undo_redo_stack(
        self, real_application_model_with_plot, real_command_manager
    ):
        """
        ...
        assert len(command_manager._undo_stack) > 0 and not (
            len(command_manager._redo_stack) > 0
        )
        ...
        cmd2_geometries = {
            plot_node.id: (
                cmd2_new_x,
                plot_node.geometry[1],
                plot_node.geometry[2],
                plot_node.geometry[3],
            )
        }
        cmd2 = BatchChangePlotGeometryCommand(
            model=model, new_geometries=cmd2_geometries, description="Move Plot 1"
        )
        ...
        assert (
            len(command_manager._undo_stack) > 0
            and len(command_manager._redo_stack) > 0
        )

        # Undo 2: Revert title
        command_manager.undo()
        assert plot_node.plot_properties.title == initial_title
        assert not (len(command_manager._undo_stack) > 0) and (
            len(command_manager._redo_stack) > 0
        )
