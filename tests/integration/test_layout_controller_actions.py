...
from src.models.layout.layout_config import GridConfig, Margins # Added for type hinting and assertions
from src.services.commands.change_grid_parameters_command import ChangeGridParametersCommand


def test_full_grid_layout_cycle(real_application_model, real_command_manager, real_layout_manager, real_layout_controller):
    """
    ...
    initial_grid_config = model.current_layout_config # This is now a GridConfig instance
    ...
    assert model.current_layout_config.cols == 1 # cols should be 1 (reverted from 1 to 1)
    assert model.current_layout_config.rows == 2 # rows should still be 2

    command_manager.undo() # Undo the 'rows' change
    assert model.current_layout_config.rows == initial_grid_config.rows # rows should revert to initial (1)
    assert model.current_layout_config.cols == initial_grid_config.cols # cols should still be 1
...
def test_free_form_alignment_undo_redo(real_application_model, real_command_manager, real_layout_manager, real_layout_controller):
    ...
    assert plot1.geometry[0] == initial_plot1_geometry[0] # plot1 x should not change if it's the leftmost
    assert plot2.geometry[0] == plot1.geometry[0] # plot2 x should align with plot1
    assert plot3.geometry[0] == initial_plot3_geometry[0] # plot3 not selected, should not change
...
def test_layout_controller_align_selected_plots_integration(real_application_model, real_command_manager, real_layout_manager, real_layout_controller, mocker):
    """
    ...
    mock_perform_align = mocker.patch.object(layout_manager, 'perform_align', return_value={
        plot1.id: (0.1, 0.1, 0.2, 0.2),
        plot2.id: (0.1, 0.5, 0.2, 0.2)
    })
...
def test_layout_controller_distribute_selected_plots_integration(real_application_model, real_command_manager, real_layout_manager, real_layout_controller, mocker):
    """
    ...
    # It should return a dict of new geometries
    mock_perform_distribute = mocker.patch.object(layout_manager, 'perform_distribute', return_value={
