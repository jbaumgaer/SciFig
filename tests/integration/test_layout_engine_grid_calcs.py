...
def test_grid_layout_engine_calculates_geometries(real_grid_layout_engine):
    ...
    geometries, calculated_margins, calculated_gutters = engine.calculate_geometries(plots, config, use_constrained_optimization=False)
    ...
    geometries_2x1, calculated_margins_2x1, calculated_gutters_2x1 = engine.calculate_geometries(plots, config_2x1, use_constrained_optimization=False)
...
def test_grid_layout_engine_constrained_layout_optimization(mock_config_service):
    ...
    config = GridConfig(
        ...
        row_ratios=[1],
        col_ratios=[1, 1],
        margins=Margins(left=0.1, right=0.1, top=0.1, bottom=0.1), # These are input, will be overridden by constrained_layout
        gutters=Gutters(hspace=[0.05], wspace=[0.05]) # These are input, will be overridden by constrained_layout
    )

    geometries, calculated_margins, calculated_gutters = engine.calculate_geometries(plots, config, use_constrained_optimization=True)
    ...
    assert plot1_geom[2] == pytest.approx(plot2_geom[2], abs=1e-3) # widths should be similar due to 1:1 ratio
...
def test_grid_layout_engine_custom_ratios(real_grid_layout_engine):
    ...
    assert plot2_geom[0] == pytest.approx(config.margins.left + plot1_expected_width + gutter_width)
...
def test_grid_layout_engine_list_gutters(real_grid_layout_engine):
    ...
    config = GridConfig(
        ...
        col_ratios=[1, 1],
        margins=Margins(left=0.1, right=0.1, top=0.1, bottom=0.1),
        gutters=Gutters(hspace=[0.02, 0.03], wspace=[0.04]) # Hspace has two values (between rows), wspace has one (between cols)
    )

    geometries, _, _ = engine.calculate_geometries(plots, config, use_constrained_optimization=False)
    ...
    assert abs(plot1_geom[1] - (0.1 + (0.8 - 0.02 - 0.03)/2 + 0.03)) < tolerance # y (bottom margin + h1 + hspace2)
    assert abs(plot1_geom[2] - ((0.8 - 0.04)/2)) < tolerance # width
    assert abs(plot1_geom[3] - ((0.8 - 0.02 - 0.03)/2)) < tolerance # height

    # Top-right plot
    assert abs(plot2_geom[0] - (0.1 + plot1_geom[2] + 0.04)) < tolerance # x (left margin + w1 + wspace)
