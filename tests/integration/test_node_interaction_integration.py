...
@pytest.fixture
def real_node_controller(real_application_model, real_command_manager, mock_properties_ui_factory):
    """Provides a real NodeController instance."""
    return NodeController(real_application_model, real_command_manager, mock_properties_ui_factory)

@pytest.fixture
def real_tool_service():
    """Provides a real ToolService with a SelectionTool."""
    tool_service = ToolService()
    selection_tool = SelectionTool(MagicMock(), MagicMock(), MagicMock(), MagicMock()) # Requires specific mocks
...
def test_node_controller_updates_model_on_property_change(real_application_model, real_command_manager, real_node_controller):
    """
...
def real_tool_service_with_mock_tools():
    ...
    mock_canvas_controller_for_tool = MagicMock(spec=CanvasController) # Mock controller for tool
    mock_app_model_for_tool = MagicMock(spec=ApplicationModel) # Mock model for tool
    mock_command_manager_for_tool = MagicMock(spec=CommandManager) # Mock command manager for tool
...
def test_canvas_controller_dispatches_mouse_event_to_tool_service(mock_canvas_controller, real_tool_service_with_mock_tools):
    """
    ...
    pos = QPointF(10, 20)
    event = QMouseEvent(QMouseEvent.MouseButtonPress, pos, Qt.MouseButton.LeftButton, Qt.MouseButton.LeftButton, Qt.KeyboardModifier.NoModifier)
    ...
    tool_service.active_tool.mouse_press_event(event)
    tool_service.get_tool("another_tool").mouse_press_event.assert_called_once_with(event)
...
def real_selection_tool(real_application_model):
    ...
    mock_canvas_widget.map_to_scene_coords.side_effect = lambda pos: pos # Simple 1:1 mapping for test
    ...
    mock_renderer.hit_test.side_effect = lambda x, y, exclude_nodes=None: None # Default: no hit

    return SelectionTool(real_application_model, mock_canvas_widget, MagicMock(), mock_renderer)


def test_selection_tool_updates_application_model_selection(real_application_model, real_selection_tool):
    """
    ...
    selection_tool.renderer.hit_test.side_effect = lambda x, y, exclude_nodes=None: plot1 if 0.1 < x < 0.3 and 0.1 < y < 0.3 else None

    # Simulate click on plot1
    event = QMouseEvent(QMouseEvent.MouseButtonPress, QPointF(0.2, 0.2), Qt.MouseButton.LeftButton, Qt.MouseButton.LeftButton, Qt.KeyboardModifier.NoModifier)
    selection_tool.mouse_press_event(event)
    event = QMouseEvent(QMouseEvent.MouseButtonRelease, QPointF(0.2, 0.2), Qt.MouseButton.LeftButton, Qt.MouseButton.LeftButton, Qt.KeyboardModifier.NoModifier)
    ...
    selection_tool.renderer.hit_test.side_effect = lambda x, y, exclude_nodes=None: None # No single hit during drag

    # Start drag at (0.05, 0.05)
    press_event = QMouseEvent(QMouseEvent.MouseButtonPress, QPointF(0.05, 0.05), Qt.MouseButton.LeftButton, Qt.MouseButton.LeftButton, Qt.KeyboardModifier.NoModifier)
    selection_tool.mouse_press_event(press_event)

    # Drag to (0.85, 0.85)
    move_event = QMouseEvent(QMouseEvent.MouseMove, QPointF(0.85, 0.85), Qt.MouseButton.LeftButton, Qt.MouseButton.LeftButton, Qt.KeyboardModifier.NoModifier)
    selection_tool.mouse_move_event(move_event) # This would typically update selection rectangle visual
    
    release_event = QMouseEvent(QMouseEvent.MouseButtonRelease, QPointF(0.85, 0.85), Qt.MouseButton.LeftButton, Qt.MouseButton.LeftButton, Qt.KeyboardModifier.NoModifier)
    ...
    def mock_hit_test_rect(rect_x, rect_y, rect_width, rect_height):
        ...
        for node in [plot1, plot2]:
            ...
            if (rect_x < node_x + node_w and rect_x + rect_width > node_x and
                rect_y < node_y + node_h and rect_y + rect_height > node_y):
                selected_nodes.append(node)
        return selected_nodes
    
    # Patch the selection_tool.renderer.hit_test_rectangle to simulate selection
    with patch.object(selection_tool.renderer, 'hit_test_rectangle', side_effect=mock_hit_test_rect):
...
def test_application_model_emits_model_changed_renders(real_application_model, mock_renderer):
    """
...
def test_application_model_selection_changed_updates_node_controller(real_application_model, real_node_controller, mock_properties_ui_factory):
    """
