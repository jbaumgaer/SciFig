...
def assembler(
    ...
    # mock_LayoutManager_class_patch = mocker.patch('src.core.composition_root.LayoutManager') # REMOVED
    ...
    # mock_LayoutManager_class_patch.return_value = mock_layout_manager # REMOVED
    ...
    # These assignments ensure that when a test calls assembler._model, it gets the fixture's mock_model
    ...
    # composition_root_instance._layout_manager = mock_layout_manager # REMOVED
    ...
    # composition_root_instance._mock_LayoutManager_class = mock_LayoutManager_class_patch # REMOVED
...
def test_assemble_menus_assigns_correctly(assembler, mock_project_controller, mock_layout_controller, mock_command_manager):
    """
    Test that _assemble_menus correctly assigns _menu_bar and _main_menu_actions
    from the MenuBarBuilder's build() method.
    """
    mock_menu_bar = assembler._menu_bar
    mock_main_menu_actions = assembler._main_menu_actions
    ...
    assembler._mock_menu_bar_builder_class.assert_called_once_with(
...
def test_assemble_returns_application_components(assembler):
    """
    ...
    assert components.project_controller is assembler._project_controller
    assert components.layout_controller is assembler._layout_controller
    assert components.node_controller is assembler._node_controller
    ...
    assert components.main_menu_actions is assembler._main_menu_actions
    assert components.tool_bar_actions is assembler._tool_bar_actions
    assert components.layout_ui_factory is assembler._layout_ui_factory
    assert components.layout_manager is assembler._layout_manager

def test_assemble_core_components_uses_config_for_figure(assembler, mock_model, mock_config_service, mocker):
    """
...
def test_assemble_main_window_receives_config_service(assembler, mock_main_window, mock_config_service, mocker):
    """
    ...
    assembler._assemble_core_components()
    assembler._assemble_menus()
    assembler._assemble_tooling()
    assembler._assemble_main_window()

    # Now verify the call to MainWindow's constructor using the patched class
    mock_main_window_class.assert_called_once_with(
        model=assembler._model,
        project_controller=assembler._project_controller,
        layout_controller=assembler._layout_controller,
        node_controller=assembler._node_controller,
        command_manager=assembler._command_manager,
        plot_types=assembler._plot_types,
        menu_bar=assembler._menu_bar,
        ...
        layout_ui_factory=assembler._layout_ui_factory,
    )

def test_controllers_receive_config_service(assembler, mock_model, mock_command_manager, mock_config_service, mock_layout_manager):
    """
    ...
    assembler._mock_ProjectController_class.assert_called_once_with(
        model=mock_model,
        command_manager=mock_command_manager,
        config_service=mock_config_service,
    ...
    assembler._mock_LayoutController_class.assert_called_once_with(
        model=mock_model,
        command_manager=mock_command_manager,
        layout_manager=mock_layout_manager
    )
    ...
    assembler._mock_NodeController_class.assert_called_once_with(
        model=mock_model,
        command_manager=mock_command_manager
    )

def test_assemble_tooling_adds_tools_and_sets_active_tool(assembler, mock_tool_manager, mock_selection_tool, mock_config_service):
    """
...
def test_assemble_tooling_assigns_canvas_widget_to_tools(assembler, mock_main_window, mock_tool_manager, mock_selection_tool):
    """
    Test that the canvas_widget is correctly assigned to the tools after MainWindow is assembled.
    """
    assembler._assemble_core_components() # Needed to initialize _model etc.
    assembler._assemble_tooling() # Tools are created, selection_tool.canvas_widget is initially None
    assembler._assemble_main_window() # MainWindow is created, and canvas_widget is assigned to tools
...
def test_assemble_canvas_controller_instantiates_canvas_controller(assembler, mock_canvas_controller, mock_model, mock_main_window, mock_tool_manager, mock_command_manager, mock_layout_controller):
    """
    Test that _assemble_canvas_controller instantiates the CanvasController with correct dependencies.
    """
    assembler._assemble_main_window() # Ensure _view and canvas_widget are set
    assembler._assemble_tooling() # Ensure _tool_manager is set
    assembler._assemble_canvas_controller()

    assembler._mock_CanvasController_class.assert_called_once_with(
        model=mock_model,
    ...
    assert assembler._canvas_controller is mock_canvas_controller
def test_connect_signals(assembler, mock_model, mock_main_window, mock_project_controller, mock_layout_controller, mock_selection_tool):
    """
    ...
    mock_model.modelChanged.connect.assert_called_once_with(
        assembler._redraw_canvas_callback
    )
...
def test_redraw_canvas_callback(assembler, mock_renderer, mock_model, mock_main_window):
    """
    ...
    assembler._assemble_core_components()
    assembler._assemble_main_window() # To set _view and canvas_widget
...
def test_properties_ui_factory_registers_builders(assembler, mock_properties_ui_factory):
    """
    ...
    mock_properties_ui_factory.register_builder.assert_any_call(
        PlotType.LINE, assembler._mock_build_line_plot_ui_widgets_func
    )
    mock_properties_ui_factory.register_builder.assert_any_call(
        PlotType.SCATTER, assembler._mock_build_scatter_plot_ui_widgets_func
    )
...
def test_composition_root_layout_component_wiring(assembler, mock_model, mock_command_manager,
    ...
    assembler._mock_FreeLayoutEngine_class.assert_called_once_with()

    # Verify GridLayoutEngine instantiation
    assembler._mock_GridLayoutEngine_class.assert_called_once_with(config_service=mock_config_service)

    # Verify LayoutManager instantiation
    assembler._mock_LayoutManager_class.assert_called_once_with(
    ...
    assembler._mock_LayoutUIFactory_class.assert_called_once_with(
        config_service=mock_config_service,
        layout_manager=mock_layout_manager,
    )
    ...
    assembler._mock_Renderer_class.assert_called_once_with(layout_manager=mock_layout_manager, application_model=mock_model)

    # Verify MainWindow receives LayoutUIFactory and LayoutManager
    assembler._mock_MainWindow_class.assert_called_once_with(
        model=mock_model,
        project_controller=assembler._project_controller,
        layout_controller=assembler._layout_controller,
        node_controller=assembler._node_controller,
    ...
    assembler._mock_CanvasController_class.assert_called_once_with(
        ...
        tool_manager=mock_tool_manager,
        command_manager=mock_command_manager,
        layout_controller=assembler._layout_controller, # Use the stored instance from assembler
    )
    assert components.canvas_controller is mock_canvas_controller

def test_composition_root_initial_layout_mode_reflection(assembler, mock_model, mock_config_service, mock_layout_manager, mocker):
    """
